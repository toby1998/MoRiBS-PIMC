...
Please note that one should delete the following files:
	1: yw001.*;
	2: permutation,
in the working directory before running a simulation. Otherwise, the simulation will bomb out.
...
A linear rotor-particle potential file should have the following format:
        ...
        2001        1001   # numbers of radial and angular grids
...
Note that all those # comments should be removed in an actual potential file. The potentials should be arranged in the following order:
	...
	loop over r grids
		loop over cos(theta) grids

A non-linear rotor-particle potential file should have the following format:

501 181 181 4.0 20.0  # numbers of radial, polar angular, and azimuthal angular grids, initial and final r values. Note that the polar angular grid should be fixed to be 181, meaning from 0 to 180 degrees. The azimuthal angular grid can be 91, 181, and 361, depending on the symmetry of the rotor. For a C2v type rotor, 91 is fine. For a Cs type rotor, 181 is fine. For most rotors, 361 should be used. Note that 181 is coded only for Cs type rotor. A C2 type rotor still requires 361.
...
The potentials should be arranged following
	...
	loop over radial grids
		loop over polar angular grids
			loop over azimuthal angular grids
...
One may compile the code, move the resultant executable "pimc" to examples/MF_8He_0.37K_512_128 or examples/N2O_5pH2_512_128, and try running the respective simulations. One may also look at the files in those two example directories to have a sense of how many files are needed and their formats.
